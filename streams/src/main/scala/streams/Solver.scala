...
trait Solver extends GameDef {
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.toList.filter(x => done(x._1) ).toStream
   ...
   * position.
   */
  lazy val solution: List[Move] = if (pathsToGoal.isEmpty) List[Move]() else pathsToGoal.head._2.reverse
}
