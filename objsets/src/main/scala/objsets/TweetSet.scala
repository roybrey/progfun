...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    val max1 = if (!left.isEmpty ) maxTweets(elem, left.mostRetweeted) else elem
    val max2 = if (!right.isEmpty ) maxTweets(elem, right.mostRetweeted) else elem
    maxTweets(max1,max2)
  }
  ...
  def maxTweets(a: Tweet, b: Tweet): Tweet = if (a.retweets > b.retweets) a else b
}
